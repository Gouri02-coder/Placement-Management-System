...
export class CustomValidators {
  ...
  static confirmPassword(controlName: string): ValidatorFn {
  ...
  // CGPA validator
  static cgpa(control: AbstractControl): ValidationErrors | null {
    if (!control.value) return null;
...
export const getValidationMessage = (errors: ValidationErrors): string => {
  const firstError = Object.keys(errors)[0];
  return VALIDATION_MESSAGES[firstError as keyof typeof VALIDATION_MESSAGES] || 'Invalid value';
};